                ...
                <div class="table-responsive">
		           <form method="post" id="searchForm">
		               	...
		               	<div class="row d-flex justify-content-between">
	                		<div class="col-3">
								<select class="form-select" id="selectDept" name="selectDept" aria-label="Default select example" style="width:113px;">
				                	<c:forEach items="${comList}" var="comList">
					                	<c:if test="${fn:startsWith(comList.comCode, 'a') }">
											<option value="${comList.comCode}" <c:if test="${selectDept eq comList.comCode }">selected</c:if>>${comList.comName}</option>
					                	</c:if>
								    </c:forEach>
								</select>
							</div>
